...
- Entity: User, Token, PatientProfile, DoctorProfile, Role, Permission
- Repository: UserRepository, TokenRepository, PatientProfileRepository, DoctorProfileRepository
...
3. **Tách Repository cho từng Entity**
   - `UserRepository`, `TokenRepository`, `PatientProfileRepository`, `DoctorProfileRepository`
...
- **Vị trí áp dụng**: Các class `UserRepository`, `TokenRepository`, v.v.
...
### 4. Builder Pattern
- **Vị trí áp dụng**: Trong các entity và DTO
...
### 4. Tách biệt Token management
- **Vấn đề ban đầu**: Logic quản lý token nằm trong `AuthenticationService`
- **Giải pháp**: Đã tách ra `TokenService` riêng biệt
- **Lợi ích**: Tuân thủ SRP, dễ dàng tái sử dụng logic token ở các service khác, dễ test hơn
...
Những cải tiến này không chỉ tăng chất lượng code mà còn tăng khả năng ứng phó với các yêu cầu thay đổi trong tương lai, làm nền tảng vững chắc cho sự phát triển bền vững của phần mềm.
